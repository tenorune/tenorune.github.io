...
- **One-button refresh:** the `refresh saves + hydrate + rebuild` workflow runs `bsky-saves fetch`, `bsky-saves hydrate articles`, `bsky-saves hydrate threads`, then `build_curator_data.py` + `build_pending_data.py`, and commits the result as a single commit. This is the default routine ingestion run; use it whenever the curator is doing a normal "what's new since last time" pull.
