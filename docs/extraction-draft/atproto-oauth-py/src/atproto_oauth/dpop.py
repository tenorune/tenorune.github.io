"""DPoP + JWK helpers for AT Protocol OAuth.

Provides:

  * ``generate_dpop_key()``: EC P-256 keypair (per RFC 9449 §3.1).
  * ``key_to_jwk(key)`` / ``jwk_to_key(jwk)``: serialize/deserialize.
  * ``public_jwk(jwk)``: strip ``d`` from a private JWK.
  * ``make_dpop_proof(...)``: build a DPoP proof JWT for one HTTP request.
  * ``dpop_post_form(...)`` / ``dpop_get(...)``: convenience wrappers that
    handle the ``use_dpop_nonce`` retry dance.
"""
...
def key_to_jwk(private_key) -> dict:
    """Serialize an EC P-256 private key as a JWK dict (with ``d`` for private)."""
...
def dpop_post_form(url: str, form: dict, private_key, pub_jwk: dict) -> dict:
    """POST form-encoded data to ``url`` with a DPoP proof.

    Retries once with a DPoP-Nonce if the server returns ``use_dpop_nonce``.
    Returns the response JSON. Raises RuntimeError on non-2xx with a snippet
    of the server's response body.
    """
    nonce: str | None = None
    for _ in range(2):
...
def dpop_get(
    ...
    """GET a protected resource with a DPoP-bound access token.

    Retries once on ``use_dpop_nonce``. Returns the final Response (caller
    decides what to do with status codes).
    """
    nonce: str | None = None
    for _ in range(2):
        proof = make_dpop_proof(
            private_key, pub_jwk, "GET", url, nonce=nonce, access_token=access_token,
        )
        r = httpx.get(
            url,
            params=params,
