...
**Status values (current implementation, 2026-04-30):**

| Value | Meaning | Frontmatter (when story exists) |
|---|---|---|
| (absent) | Not yet processed; implicitly pending. | n/a — no story file |
| `queued` | Curator clicked "Queue" on the pending dashboard; flagged for the next bulk-draft. | n/a |
| `skipped` | Curator clicked "Skip"; do not draft. | n/a |
| `drafted` | Story file created with Claude's untouched prose. | `published: false` |
| `polished` | Curator edited the prose; still not public. (Reserved; no UI emits it yet.) | `published: false` |
| `published` | Live on the site. | `published: true` |
| `culled` | Curator clicked "Reject"; story is hidden but the file is preserved (so the rejection is recorded and not re-drafted). | `published: false`, `culled: true` |

`story_slug` is null for `queued`/`skipped`, present for `drafted`/`polished`/`published`/`culled`.

**State transitions:**

```
                  curator UI                 curator UI / chat
              (skip / queue)              (publish / draft / reject)
                     │                                │
pending ──┬──→ skipped                                │
          │                                           ▼
          ├──→ queued ──→ Claude drafts ──→  drafted  ↔  published
          │                                     │           │
          └──→ Claude drafts (no queue) ────────┤           │
                                                ▼           │
                                              culled  ←─────┘
                                            (file kept,
                                             `culled: true`)
...
- `_stories/*.md` frontmatter (`published`, `culled`) is the source of truth for what the **public site** renders. Jekyll filters `published: false` out of `site.stories` natively.
- `saves_state.json` mirrors that frontmatter for editorial reporting; `scripts/curate.py` updates both when a `publish`/`draft`/`reject` action lands.
- The curator dashboard reads `_data/curator.yml` (regenerated by `scripts/build_curator_data.py`), which derives the displayed state from frontmatter — so even if `saves_state` drifts, the dashboard stays accurate.
- Pending-side actions (`queue`/`skip`/`unqueue`) only touch `saves_state`; they have no story file to mutate.

**Authoring default:** new drafts (whether bulk-drafted by Claude in chat or processed via the drain workflow) ship with `published: false`. Promotion to `published: true` is a separate curator action.
